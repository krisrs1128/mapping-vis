...
write_patches <- function(x_path, ys, centers, out_dir, imsize=512) {
  unlink(out_dir)
  dir.create(out_dir)
  ...
  for (i in seq_len(nrow(centers))) {
    ...
    if (!is.null(patch)) {
      margin <- (dim(patch$x)[1] - imsize) / 2
      ix <- seq(margin, margin + imsize - 1)
      y <- label_mask(ys, patch$raster)[ix, ix, ]
      x <- patch$x[ix, ix, ]
